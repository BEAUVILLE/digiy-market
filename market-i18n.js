/* DIGIY MARKET — moteur métier multilingue stable · 7 langues · bascule par rechargement */
(function(){
'use strict';

var B=window.DIGIY_MARKET_PACK_BASE;
if(!B)return;

var sourceLangs={
  es:window.DIGIY_MARKET_LANG_ES,
  de:window.DIGIY_MARKET_LANG_DE,
  it:window.DIGIY_MARKET_LANG_IT,
  nl:window.DIGIY_MARKET_LANG_NL,
  ar:window.DIGIY_MARKET_LANG_AR
};
var D={sources:B.sources,locales:B.locales,rows:{},meta:{}};
Object.keys(sourceLangs).forEach(function(lang){
  var item=sourceLangs[lang];
  if(item&&Array.isArray(item.row)){
    D.rows[lang]=item.row;
    D.meta[lang]=item.meta||{};
  }
});

var SUPPORTED=['fr','en','es','de','it','nl','ar'];
var LABELS={fr:'🇫🇷 FR',en:'🇬🇧 EN',es:'🇪🇸 ES',de:'🇩🇪 DE',it:'🇮🇹 IT',nl:'🇳🇱 NL',ar:'🌙 AR'};
var INTERNAL_KEY='digiy_market_lang_v2';
var VERSION='20260730-market7g';
var registered=false;
var scheduled=false;
var applying=false;
var clickGuardInstalled=false;
var officialProfilesLoaded=false;
var officialProfiles={};

function selectedLanguage(){
  try{
    var q=(new URL(location.href)).searchParams.get('lang');
    q=String(q||'').slice(0,2).toLowerCase();
    if(SUPPORTED.indexOf(q)>=0)return q;
    var saved=String(localStorage.getItem('digiy-lang')||'').slice(0,2).toLowerCase();
    if(SUPPORTED.indexOf(saved)>=0)return saved;
  }catch(e){}
  return 'fr';
}

function buildPacks(){
  var out={};
  Object.keys(D.rows).forEach(function(lang){
    var row=D.rows[lang],pack={};
    D.sources.forEach(function(source,index){pack[source]=row[index];});
    out[lang]=pack;
  });
  return out;
}

function registerTranslations(){
  if(registered)return;
  if(window.DIGIY_I18N&&typeof window.DIGIY_I18N.register==='function'){
    registered=true;
    window.DIGIY_I18N.register(buildPacks());
  }
}

function setMeta(){
  var lang=selectedLanguage();
  var meta=D.meta[lang];
  document.documentElement.lang=lang;
  document.documentElement.dir=lang==='ar'?'rtl':'ltr';
  if(!meta)return;
  document.title=meta.title;
  var desc=document.querySelector('meta[name="description"]');
  if(desc)desc.setAttribute('content',meta.description);
}

function injectLayoutFix(){
  if(document.getElementById('market-language-layout-fix'))return;
  var style=document.createElement('style');
  style.id='market-language-layout-fix';
  style.textContent=
    '#digiy-i18n-bar{display:none!important}'+
    '.digiy-langbar{display:grid!important;grid-template-columns:repeat(7,minmax(0,1fr))!important;width:min(100%,680px)!important;max-width:100%!important;gap:5px!important;overflow:visible!important;position:relative!important;z-index:100!important;flex:1 1 100%!important;pointer-events:auto!important}'+
    '.digiy-langbar .digiy-langbtn{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:0!important;min-height:40px!important;padding:0 7px!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;touch-action:manipulation!important}'+
    '@media(min-width:761px){.topbar-inner{flex-wrap:wrap!important}.digiy-langbar{order:3!important;margin-left:auto!important;margin-top:3px!important}}'+
    '@media(max-width:760px){.digiy-langbar{order:2!important;width:100%!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;margin:6px 0 0!important}.digiy-langbar .digiy-langbtn{font-size:.76rem!important;padding:0 5px!important}.top-actions{order:3!important}}'+
    '@media(max-width:390px){.digiy-langbar .digiy-langbtn{font-size:.70rem!important;padding:0 3px!important}}';
  document.head.appendChild(style);
}

function navigateToLanguage(lang){
  if(SUPPORTED.indexOf(lang)<0)lang='fr';
  try{
    localStorage.setItem('digiy-lang',lang);
    if(lang==='fr')localStorage.removeItem(INTERNAL_KEY);
    else localStorage.setItem(INTERNAL_KEY,'en');
  }catch(e){}
  try{if(window.speechSynthesis)window.speechSynthesis.cancel();}catch(e){}
  var target='https://market.digiylyfe.com/?lang='+encodeURIComponent(lang)+'&v='+encodeURIComponent(VERSION);
  location.assign(target);
}

function renderLanguageBar(){
  var bar=document.querySelector('.digiy-langbar');
  if(!bar)return;
  var lang=selectedLanguage();
  if(bar.dataset.marketLangVersion!==VERSION){
    bar.dataset.marketLangVersion=VERSION;
    bar.innerHTML='';
    SUPPORTED.forEach(function(code){
      var button=document.createElement('button');
      button.className='digiy-langbtn';
      button.type='button';
      button.dataset.digiyLang=code;
      button.textContent=LABELS[code];
      button.addEventListener('click',function(event){
        event.preventDefault();
        event.stopImmediatePropagation();
        navigateToLanguage(code);
      },true);
      bar.appendChild(button);
    });
  }
  bar.setAttribute('aria-label',lang==='ar'?'اختيار اللغة':'Choisir la langue');
  bar.querySelectorAll('.digiy-langbtn[data-digiy-lang]').forEach(function(button){
    var active=button.dataset.digiyLang===lang;
    button.classList.toggle('active',active);
    button.setAttribute('aria-pressed',active?'true':'false');
  });
}

function contactMessage(shop,channel){
  var meta=D.meta[selectedLanguage()];
  if(!meta)return null;
  var template=channel==='sms'?meta.sms:meta.whatsapp;
  return template.replace('{shop}',shop||'DIGIY MARKET');
}

function patchContacts(){
  var lang=selectedLanguage();
  if(lang==='fr'||lang==='en')return;
  document.querySelectorAll('.shop-card').forEach(function(card){
    var title=card.querySelector('.shop-title');
    var shop=title?title.textContent.trim():'DIGIY MARKET';
    var wa=card.querySelector('a.btn-live[href*="wa.me"]');
    if(wa){
      try{
        var waUrl=new URL(wa.href);
        waUrl.searchParams.set('text',contactMessage(shop,'whatsapp'));
        wa.href=waUrl.toString();
      }catch(e){}
    }
    var sms=card.querySelector('a.btn-soft[href^="sms:"]');
    if(sms){
      var phone=(sms.getAttribute('href')||'').split('?')[0];
      sms.setAttribute('href',phone+'?body='+encodeURIComponent(contactMessage(shop,'sms')));
    }
  });
}

function profileKey(value){
  return String(value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ').trim();
}

function loadOfficialProfiles(){
  if(officialProfilesLoaded)return Promise.resolve(officialProfiles);
  officialProfilesLoaded=true;
  return fetch('./products.json?v='+VERSION,{cache:'no-store'})
    .then(function(response){if(!response.ok)throw new Error('products');return response.json();})
    .then(function(data){
      var sellers=Array.isArray(data&&data.active_sellers)?data.active_sellers:[];
      sellers.forEach(function(seller){
        var url=String(seller.profile_url||'').trim();
        if(!url)return;
        var nameKey=profileKey(seller.name);
        var idKey=profileKey(seller.id);
        if(nameKey)officialProfiles[nameKey]=url;
        if(idKey)officialProfiles[idKey]=url;
      });
      return officialProfiles;
    })
    .catch(function(){return officialProfiles;});
}

function directProfileUrl(value){
  try{
    var url=new URL(value,location.href);
    if(/\/shop\.html$/i.test(url.pathname))url.pathname='/fiche.html';
    if(url.hostname==='market.digiylyfe.com'&&/\/fiche\.html$/i.test(url.pathname)){
      var lang=selectedLanguage();
      if(lang!=='fr')url.searchParams.set('lang',lang);
      else url.searchParams.delete('lang');
      return url.toString();
    }
  }catch(e){}
  return value||'';
}

function profileUrlForLink(link){
  var card=link&&link.closest?link.closest('.shop-card'):null;
  var title=card&&card.querySelector('.shop-title');
  var official=officialProfiles[profileKey(title?title.textContent:'')];
  return official||directProfileUrl(link.getAttribute('href')||link.href);
}

function patchProfileLinks(){
  document.querySelectorAll('.shop-card .shop-actions a.btn-main').forEach(function(link){
    var fixed=profileUrlForLink(link);
    if(fixed)link.href=fixed;
    link.target='_self';
    link.dataset.marketProfileDirect='1';
  });
}

function installProfileClickGuard(){
  if(clickGuardInstalled)return;
  clickGuardInstalled=true;
  document.addEventListener('click',function(event){
    var link=event.target&&event.target.closest?event.target.closest('.shop-card .shop-actions a.btn-main'):null;
    if(!link)return;
    var fixed=profileUrlForLink(link);
    if(!fixed)return;
    event.preventDefault();
    event.stopImmediatePropagation();
    location.assign(fixed);
  },true);
}

function patchShare(){
  var meta=D.meta[selectedLanguage()];
  if(!meta)return;
  ['bottomShare','fastShare','btnHeroShare'].forEach(function(id){
    var element=document.getElementById(id);
    if(!element||element.dataset.marketShareSeven==='1')return;
    element.dataset.marketShareSeven='1';
    element.addEventListener('click',function(event){
      var lang=selectedLanguage();
      if(lang==='fr'||lang==='en')return;
      event.preventDefault();
      event.stopImmediatePropagation();
      var payload={title:document.title,text:meta.share,url:location.href};
      if(navigator.share)navigator.share(payload).catch(function(){});
      else if(navigator.clipboard)navigator.clipboard.writeText(location.href).catch(function(){});
    },true);
  });
}

function speakCurrentLanguage(){
  var lang=selectedLanguage();
  var meta=D.meta[lang];
  if(!meta||!('speechSynthesis' in window))return;
  var utterance=new SpeechSynthesisUtterance(meta.audio);
  utterance.lang=D.locales[lang]||'fr-FR';
  utterance.rate=.86;
  var voices=window.speechSynthesis.getVoices()||[];
  var prefix=utterance.lang.slice(0,2).toLowerCase();
  var voice=voices.find(function(item){return String(item.lang||'').toLowerCase().indexOf(prefix)===0;});
  if(voice)utterance.voice=voice;
  window.speechSynthesis.cancel();
  window.speechSynthesis.speak(utterance);
}

function patchAudio(){
  var play=document.getElementById('digiyAudioPlayBtn');
  var stop=document.getElementById('digiyAudioStopBtn');
  if(play&&play.dataset.marketAudioSeven!=='1'){
    play.dataset.marketAudioSeven='1';
    play.addEventListener('click',function(event){
      var lang=selectedLanguage();
      if(lang==='fr'||lang==='en')return;
      event.preventDefault();
      event.stopImmediatePropagation();
      speakCurrentLanguage();
    },true);
  }
  if(stop&&stop.dataset.marketAudioSeven!=='1'){
    stop.dataset.marketAudioSeven='1';
    stop.addEventListener('click',function(event){
      var lang=selectedLanguage();
      if(lang==='fr'||lang==='en')return;
      event.preventDefault();
      event.stopImmediatePropagation();
      try{window.speechSynthesis.cancel();}catch(e){}
    },true);
  }
}

function patch(){
  if(applying)return;
  applying=true;
  injectLayoutFix();
  renderLanguageBar();
  setMeta();
  patchContacts();
  patchProfileLinks();
  patchShare();
  patchAudio();
  applying=false;
}

function schedulePatch(){
  if(scheduled)return;
  scheduled=true;
  setTimeout(function(){scheduled=false;patch();},30);
}

var observer=new MutationObserver(function(){
  if(!applying)schedulePatch();
});

function init(){
  injectLayoutFix();
  renderLanguageBar();
  installProfileClickGuard();
  registerTranslations();
  setMeta();
  loadOfficialProfiles().then(schedulePatch);
  patch();
  observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(patch,150);
  setTimeout(patch,700);
}

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});
else init();
})();