...
function ensure(){if(document.getElementById(ID))return true;const panel=document.querySelector('#boutiques > .panel');const fg=document.getElementById('fg-nails-market-featured');if(!panel||!fg)return false;style();const l=lang(),x=T[l]||T.fr;const box=document.createElement('section');box.id=ID;if(l==='ar')box.dir='rtl';box.setAttribute('aria-label','Partenaires actifs DIGIY MARKET');box.innerHTML=PARTNERS.map(p=>`<article class="fp-card"><span class="fp-label">${x.label}</span><div class="fp-grid"><a href="${withLang(p.shop,l)}" target="_blank" rel="noopener"><img src="${p.image}" alt="Carte de visite ${p.name}" loading="lazy"></a><div><h3>${p.name}</h3><p>${x[p.key]}</p><div class="fp-meta"><span>${p.meta[0]}</span><span>${p.meta[1]}</span><span>${x.commission}</span></div><div class="fp-actions"><a class="fp-main" href="${withLang(p.shop,l)}" target="_blank" rel="noopener">${x.shop}</a><a class="fp-gold" href="${withLang(p.order,l)}" target="_blank" rel="noopener">${x.order}</a><a class="fp-soft" href="${p.wa}" target="_blank" rel="noopener">WhatsApp</a></div></div></div></article>`).join('');fg.insertAdjacentElement('afterend',box);return true}
